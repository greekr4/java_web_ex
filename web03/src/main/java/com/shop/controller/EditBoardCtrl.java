...
public class EditBoardCtrl extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ((DAO.editBoard(Vo) > 0)){
			//글쓰기 성공
			out.println("<script>alert('수정성공'); location.href='GetBoardListCtrl';</script>");
		} else {
			//글쓰기 실패
			out.println("<script>alert('수정실패'); location.href='GetBoardListCtrl'; </script>");
		}	
	}
}
